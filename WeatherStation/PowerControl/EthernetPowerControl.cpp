...
static void write_PHY(unsigned int PhyReg, unsigned short Value) {
	...
	unsigned int tout;
	/* Hardware MII Management for LPC176x devices. */LPC_EMAC ->MADR = DP83848C_DEF_ADR | PhyReg;
	LPC_EMAC ->MWTD = Value;
...
void EMAC_Init() {
	...
	/* Reset all EMAC internal modules. */LPC_EMAC ->MAC1 = MAC1_RES_TX | MAC1_RES_MCS_TX | MAC1_RES_RX
	...
	/* Initialize MAC control registers. */LPC_EMAC ->MAC1 = MAC1_PASS_ALL;
	...
	/* Reset Reduced MII Logic. */LPC_EMAC ->SUPP = SUPP_RES_RMII;
	for (tout = 100; tout; tout--)
		;
	LPC_EMAC ->SUPP = 0;
...
void PHY_PowerDown() {
	if (!Peripheral_GetStatus(LPC1768_PCONP_PCENET)) EMAC_Init(); //init EMAC if it is not already init'd
